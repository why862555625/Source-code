...
2、调用 normalizePath 将 path 标准化，比较重要的是这里会将子路由的 path 父路由的 path 拼接在一起。
3、处理 caseSensitive 参数，它也是 path-to-regexp 中的参数。
4、声明一个 RouteRecord ，主要代码：103行
...
function addRouteRecord(
  ...
  if (route.children) {
    ...
    // 如果该路由存在子路由，则递归调用 addRouteRecord 添加路由记录。
  ...
  // 将这条 RouteRecord 存入 pathList 
  // 将这条记录以 path 作为 key 存入 pathMap 
  ...
  // 如果存在 name ，则以 name 作为 key 存入 nameMap
